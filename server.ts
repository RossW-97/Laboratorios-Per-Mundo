...
if (productCount.count === 0) {
  const insertProduct = db.prepare(`
    INSERT INTO products (species, variety, biotization_type, senasa_cert, stock_in_vitro, growth_speed, base_price, region)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `);

  insertProduct.run("Musáceas", "Plátano Bellaco", "Biotizado +30%", "SENASA-2024-001", 5000, "Rápido", 3.53, "Loreto");
  insertProduct.run("Aniba rosaeodora", "Palo Rosa", "Biotizado Clonal", "SENASA-2024-002", 2000, "Moderado", 6.10, "San Martín");
  insertProduct.run("Cedrela odorata L.", "Cedro Amargo", "Biotizado", "SENASA-2024-003", 1500, "Lento", 5.50, "Loreto");
  insertProduct.run("Swietenia macrophylla", "Caoba", "Biotizado", "SENASA-2024-004", 1200, "Lento", 6.50, "San Martín");
}
...
async function startServer() {
  ...
  app.get("/api/catalogo", (req, res) => {
    const { region, growth_speed } = req.query;
